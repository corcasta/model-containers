...
def train(cfg: TrainPipelineConfig, accelerator: "Accelerator | None" = None):
    ...
    print(f"OSCAR-DEBUG - RENAME_MAP: {cfg.rename_map}")
    print(f"OSCAR-DEBUG - Policy: {policy.config.input_features}")
    ...
    for _ in range(step, cfg.steps):
        start_time = time.perf_counter()
        batch = next(dl_iter)
        print(f"OSCAR-DEBUG - B-Batch: {batch}")
        batch = preprocessor(batch)
        print(f"OSCAR-DEBUG - A-Batch: {batch}")
